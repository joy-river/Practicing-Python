...
class quiz_ui:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.canvas_text = self.canvas.create_text(150, 125, width=280, text="placeholder",
        ...
        self.true_button.config(width=100, height=97, bg=THEME_COLOR, highlightthickness=0, image=self.image_true,
                                command=self.true_pressed)
        self.true_button.grid(padx=20, pady=20, row=2, column=0)
        self.false_button = Button()
        self.false_button.config(width=100, height=97, bg=THEME_COLOR, highlightthickness=0, image=self.image_false,
    ...
    def give_feedback(self, tf:bool):
        if tf:
